...
- (void)_makeTabViewCurrent:(BHTabView *)tabView {
  ...
  [self.contentView removeFromSuperview];
	...
	NSLog(@"viewcontroller bounds: %f, %f", viewController.view.bounds.size.width, viewController.view.bounds.size.height);
	NSLog(@"viewcontroller frame: %f, %f", viewController.view.frame.size.width, viewController.view.frame.size.height);
	
  self.contentView.autoresizingMask = UIViewAutoresizingFlexibleWidth|UIViewAutoresizingFlexibleHeight;
  
	CGRect contentFrame = CGRectMake(0, self.tabsContainerView.frame.size.height, viewController.view.frame.size.width, viewController.view.frame.size.height);
	
  [self.view addSubview:self.contentView];

		NSLog(@"viewcontroller frame: %f, %f", viewController.view.bounds.size.width, viewController.view.bounds.size.height);
	
		NSLog(@"content frame: %f, %f, %f, %f", self.contentView.frame.origin.x, self.contentView.frame.origin.y, self.contentView.frame.size.width, self.contentView.frame.size.height);

	//[self.view setNeedsLayout];
	
	self.view.frame = CGRectMake(self.view.frame.origin.x, self.view.frame.origin.y, self.contentView.frame.size.width, self.tabsContainerView.frame.size.height + self.contentView.frame.size.height);
