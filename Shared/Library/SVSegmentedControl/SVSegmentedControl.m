...
- (BOOL)continueTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
	...
	if((newMaxX > pMaxX || newMinX < pMinX) && self.trackingThumb) {
		self.snapToIndex = MIN(floor(self.thumb.center.x/self.segmentWidth), [self.thumbRects count]-1);
...
- (void)endTrackingWithTouch:(UITouch *)touch withEvent:(UIEvent *)event {
	...
	if(!self.moved && self.trackingThumb && [self.titlesArray count] == 2)
		...
		self.snapToIndex = MIN(floor(cPos.x/self.segmentWidth), [self.thumbRects count]-1);
		[self snap:YES];
	} 
        ...
        self.snapToIndex = MIN(floor(posX/self.segmentWidth), [self.thumbRects count]-1);
        [self snap:YES];
    }
}
...
- (void)snap:(BOOL)animated {
	...
	if(self.snapToIndex != -1)
		...
		index = MAX(0, MIN(floor(self.thumb.center.x/self.segmentWidth), [self.thumbRects count]-1));
...
- (void)moveThumbToIndex:(NSUInteger)segmentIndex animate:(BOOL)animate {
    ...
    [self sendActionsForControlEvents:UIControlEventValueChanged];
		...
		if(animate) {
			...
			if (self.selectedIndex < [self.thumbRects count])
			{
